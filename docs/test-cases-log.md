...
| V0.3 | Playwright MCP can navigate to studio.huminic.app, log in, reach `/engagements`, `/profiles`, `/agents` | playwright | PLANNED — after rebuild | |
| V0.4 | `/api/auth-session`, `/api/plugins`, `/api/engagements` respond against new live build | script | PLANNED — after rebuild | |
