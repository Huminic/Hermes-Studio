...
| GAP-CONSOLE-001 | PARTIAL FIX — PENDING-COOLIFY-REDEPLOY | Fonts half FIXED (commit `c76e844ac`): added `https://fonts.googleapis.com` to `style-src` + `https://fonts.gstatic.com` to `font-src`; the Google Fonts stylesheet now loads. The React #418 hydration warning is SEPARATE and stays OPEN (non-blocking dev-console noise). | 2026-06-01 Playwright sweep noticed | fonts done; #418 backlog | `docs/launch/evidence/blocker-fixes/low-bucket/` |
...
| GAP-CSP-META-001 | FIXED — PENDING-COOLIFY-REDEPLOY | server-entry.js now emits `Content-Security-Policy: frame-ancestors 'none'` + `X-Frame-Options: DENY` as HTTP headers (the `<meta>` form was ignored for this directive). Verified via `curl -I`. | 2026-06-01 Phase 8 live headed sweep | commit `c76e844ac` | `docs/launch/evidence/blocker-fixes/low-bucket/` |
| GAP-API-CONNECTION-STATUS-500 | FIXED — PENDING-COOLIFY-REDEPLOY | Handler returned the `isAuthenticated` boolean cast as a Response → 500. Now returns `401 {"error":"Unauthorized"}` for unauthenticated callers (verified live with HERMES_PASSWORD set). | 2026-06-01 Phase 8 live headed sweep | commit `c76e844ac` | `docs/launch/evidence/blocker-fixes/low-bucket/` |
