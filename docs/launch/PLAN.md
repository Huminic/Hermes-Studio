...
| GAP-FLOW-concurrent-edit-001 | OPEN — investigate | WF-XAC-006 says concurrent edits between customer-admin + operator on the same wiki page resolve via last-write-wins with a conflict prompt to the loser. Confirm during customer-admin-guide.md write whether a conflict-prompt UI exists or whether collisions are silent-overwrite today. If silent → flag as a launch-blocker for multi-writer wiki, OR document the single-writer convention as launch-time procedure. | 2026-06-01 WORKFLOWS.md drafting | investigate during customer-admin-guide.md write | `docs/launch/WORKFLOWS.md` WF-XAC-006 |
