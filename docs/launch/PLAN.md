...
| GAP-VER-001 | OPEN — MEDIUM | Admin sidebar is missing "Plugins" and "MCP Tokens" links. Direct nav to `/plugins` and `/mcp-tokens` both return 404. `/api/plugins` API itself works correctly (3 plugins, 0 issues). The admin UI to view + manage MCP tokens or browse plugin metadata is absent from the running studio. Fix: add `/mcp-tokens` and `/plugins` routes + sidebar items wired into existing `/api/plugins` + MCP tokens registry. | 2026-06-02 verifier PROC-001 + PROC-013 | small fix | `docs/launch/VERIFICATION_REPORT.md` |
| GAP-VER-002 | OPEN — HIGH (worse than GAP-AUTH-HYDRATION-SPLASH-001) | Direct URL navigation to admin routes (`/profiles`, `/agents`, `/engagements`, `/tasks`, `/audit`) renders a persistent Sign-in form for an authenticated session — auth-check effectively never completes on direct nav. Auth-gated content only renders via SPA navigation from `/chat/new` → sidebar link. `GET /api/auth-session` simultaneously confirms session is authenticated. GAP-AUTH-HYDRATION-SPLASH-001 said this resolves in ~3s; verifier observed it as permanent on direct nav. Effect: admins can only reach admin pages via click-through, never by bookmark. Fix: investigate auth-gate timing on direct-nav SSR vs SPA-nav code paths. | 2026-06-02 verifier PROC-010 + PROC-130 | medium investigate + fix | `docs/launch/VERIFICATION_REPORT.md` |
| GAP-VER-003 | OPEN — MEDIUM (security) | Password-reset rate-limit (claimed 3/min/IP) does NOT enforce. 5 rapid `POST /api/auth/reset-request` calls all returned HTTP 200 — zero 429s. Anti-enumeration body identical (correct), but rate cap is non-functional. Fix: verify rate-limit middleware is wired into the reset-request route in production and not just at fixture level. | 2026-06-02 verifier PROC-005 + PROC-104 | small fix | `docs/launch/VERIFICATION_REPORT.md` |
| GAP-VER-004 | OPEN — MEDIUM | `/agents` page shows ONLY 8 built-in stock agents (Roger / Sally / Bill / Ada / Max / Luna / Kai / Nova) — no profile-distributed SOULs and no custom agents from `~/.runtime/agent-definitions.json` surfaced. Spec assumed page aggregates all three sources; observed only built-ins. Fix: extend `/api/agents` to enumerate profile SOULs + custom agent-definitions in addition to built-ins. | 2026-06-02 verifier PROC-011 | medium fix | `docs/launch/VERIFICATION_REPORT.md` |
| GAP-VER-005 | OPEN — MEDIUM | `/engagements/<customer>` detail-page drilldown does NOT render. URL changes to `/engagements/huminic` but page body remains the overview list — expected detail page (stage strip / readiness gates / deployment notes / crew rosters / adjacent neighbors) does not appear. Data exists (cards show summary counts from engagement-state.yaml) but detail view is not rendering. Fix: investigate routing + detail component mount in `routes/engagements/$customer.tsx`. | 2026-06-02 verifier PROC-012 | medium investigate + fix | `docs/launch/VERIFICATION_REPORT.md` |
| GAP-VER-006 | OPEN — LOW | `/audit` page filter taxonomy differs from procedure spec. Spec expected `action_type=COMMS_FAIL`, `profile=huminic-motors` filters; actual UI shows `Tool Call / User Message / Approval` filters + time-range chips. Also `/audit` body is "No audit events found" under "All time" — either no events ever or DB pruned. Spec assumed KSG_BLOCKED / COMMS_FAIL rows would exist; none visible. Fix: back-fill spec to match UI OR extend filter to include action_type / profile dimensions. Separately: confirm audit-event write path is firing (KSG block tests in PROC-032 should have produced rows). | 2026-06-02 verifier PROC-015 + PROC-032 | small investigate | `docs/launch/VERIFICATION_REPORT.md` |
| GAP-VER-007 | OPEN — MEDIUM (deploy artifact) | Production Studio Docker image is a built dist (`/app/dist`, `/app/server-entry.js`, `/app/node_modules`, `/app/package.json` only). The `scripts/` directory is NOT shipped in the image. CLI workarounds documented in PROC-020 (`scripts/provision-launch-profiles.ts`), PROC-022 (`scripts/create-user.ts`), and PROC-103 (provision script idempotency) all rely on `docker exec ... pnpm tsx scripts/<file>.ts` — which fails at the `scripts/` lookup. At launch the operator must run these scripts from a host clone of the source repo with database access to the production volume; the documented "docker exec" workaround does not work as written. Fix: either include `scripts/` in the deployed image, OR rewrite manuals to point to the host-clone-based workflow. | 2026-06-02 verifier PROC-020 | medium operator decision | `docs/launch/VERIFICATION_REPORT.md` |
| GAP-VER-008 | OPEN — LOW | Knowledge tab tree-view DISPLAYS protected paths (`governance/`, `brain/`, `archive/`-style trees) and relies on the footer caption ("Canon/governance pages are read-only here.") + write-time KSG block to enforce read-only. Procedure spec said the tree would EXCLUDE these paths. Behavior is functionally safe (KSG blocks writes) but the spec → behavior gap may confuse customer-admins who can see and click into governance/canon files. Fix: either update spec to match (tree shows + marks read-only), OR hide protected paths from the tree entirely. | 2026-06-02 verifier PROC-031 | small fix | `docs/launch/VERIFICATION_REPORT.md` |
