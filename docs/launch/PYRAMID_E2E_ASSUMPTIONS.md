...
| D-07 | DECISION | Browser **contact-form widget UI** is a "coming soon" stub for form/voice/video modes (`w.$slug.tsx` stubBody); only `mode: chat` widgets render live in the browser. Form *ingestion* (`/api/public/widget-form`) + notification work live. Found by Dexter (independent), confirmed by Claude. My original cert (P-22/P-23) over-claimed widgets as fully live — corrected. | **RESOLVED (Option A built)** — operator chose Option A in the rolling-rollout /goal. `mode: form` now renders a real lead form (name/email/phone/message + frontmatter `greeting`/`domain`/`submit_label`/`thank_you`) that POSTs to the existing `/api/public/widget-form`; success state shown inline. Voice/video remain honest stubs (Vapi/Tavus handle those channels inbound, not the browser widget). 3 render tests added (`widget-form-render.test.ts`). | FIXED |
