...
sequenceDiagram
    ...
    participant KSG as serra-service-data-governor (KSG role)
    participant DSG as serra-service-data-governor (DSG role)
    participant FS as serra-service wiki
    participant BR as serra-service Brain
...
- **Wiki (KSG):** `~/.hermes/profiles/serra-service/{canon,governance,knowledge,archive}/**` (all writes)
- **Brain (DSG):** `~/.hermes/profiles/serra-service/brain/brain.db` (all writes)
- **Engagement state:** `~/.hermes/profiles/serra-service/engagement-state.yaml` (read-only; surfaces reconciliation candidates in deployment_notes)

## What it reads at runtime

- Every proposed wiki write to `serra-service/knowledge/`, `governance/`, `canon/`.
- Every proposed Brain write to `serra-service/brain/brain.db`.
...
- **Reconcile-needed.** DSG creates candidate; operator approves OR rejects from `/engagements/serra-service`. On approval: canon updates + DSG re-evaluates pending writes. On rejection: write rejected, audit row final.
