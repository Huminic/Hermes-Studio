...
integer value+metadata co-mutations all reject (`0` is not `false`, `1` is not `true`). Strictness also covers downstream `validate_blast_radius` and `check_vocab`'s `FCS.blast_radius_expected` vs `BLAST_SEM`: probes 54-56 substitute integers into the FCS blast ACTUAL and embedded EXPECTED for all three bool fields and reject.
