...
| 6 | Carry forward **all 17** authoritative accepted+evaluated Honda metrics (gate5b/gate2/Gate 4A); no reset to not_measured/admitted_held; planning placeholders explicitly non-authoritative | `master-ledger-295.json`; ledger carry-forward + no-regression checks |
...
| P1B.8 | Adversarial controls fire | **PASS** | **15/15** probes reject (A–O) |
...
- `python3 scripts/halo-phase1b/validate_phase1b.py --no-write` → **RESULT: PASS** (0 errors; **15/15** adversarial probes reject: A sip-as-disposition-only, B proxy-attach, C overlay-projection, D union≠295, E per-packet-reacquisition, F transition self-transition/append-only, G SIP-eval, H bucket-mismatch, I OR-for-AND, J business-hours-for-after-hours, K duration-for-event, L minutes-diff-for-share, M target-replacement, N accepted-state-regression, O cross-packet-overlap).
