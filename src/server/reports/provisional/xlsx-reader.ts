 ...
 *
 * VENDORED COPY (from `hs-ingest-dev/src/server/ingest/xlsx-reader.ts`) so the interim
 * provisional (non-promoting) analysis adapter in this isolated worktree stays
 * self-contained and depends on NOTHING in the strict ingest engine. Parse-only: it never
 * writes, promotes, or mutates the governed store, ledgers, readiness, or strict contract.
 *
