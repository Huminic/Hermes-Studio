 ...
 * Communication Log family. ONE intentional record for EVERY SW-001..SW-295.
 *
 * This is the source of truth: each metric's category, the exact admitted derivative fields it
 * uses (usually [] because this family does not satisfy the metric), missing inputs, minimum
 * history, join/NLP/external requirement, and a rationale are hand-authored per ID. The build
 * MUST reject any missing / duplicate / extra / out-of-range ID and any admitted field not in
 * DERIVATIVE_SCHEMA_FIELDS. There is NO regex/section/acquisition_class inference at build time;
 * the reusable helpers below only shape hand-chosen records. Nothing here promotes any metric.
 */
...
// Reusable authoring helpers (they only shape a hand-chosen category; they do not decide it).
...
const bound = (why: string): Dec => ({
  c: 'outside_sales_boundary',
  missing: 'Sales boundary',
  join: 'separate Service / compliance route',
  why,
})
const pend = (
  fields: Array<string>,
...
export const DECISIONS: Record<string, Dec> = {
  ...
  'SW-038': other(
    ...
    'stage skipping needs CRM stage history',
  ),
  'SW-039': other('CRM stage dwell', 'stuck-in-working needs CRM stage timing'),
  'SW-040': other(
    'CRM stage transitions',
  ...
  'SW-051': other(
    ...
    'F&I penetration is an F&I metric',
  ),
  ...
  'SW-055': other(
    'deal jacket stips',
    'missing stips is a deal-compliance/DMS metric',
  ),
  'SW-056': other(
  ...
  'SW-059': other(
    'inventory on lot',
    'demand-vs-inventory is an inventory metric',
  ),
  'SW-060': other(
  ...
  'SW-062': other(
    'inventory turn rate',
    'inventory turn is an inventory metric',
  ),
  ...
  'SW-071': nlp(
    'churn/competitor language',
    'churn language needs message content',
  ),
  'SW-072': nlp(
    'objection detection',
  ...
  // §10 Service-to-Sales & Equity Mining (Sales boundary)
  'SW-079': bound(
    'Service equity customers is Service-domain (Service-to-Sales)',
  ),
  'SW-080': bound('lease maturity + service sourcing is Service-to-Sales'),
  'SW-081': bound('Service RO customer is Service-domain'),
  'SW-082': bound('warranty/VSC is Service-domain'),
  'SW-083': bound('declined service work is Service-domain'),
  ...
  'SW-087': other(
    'BDC set-to-show',
    'set-to-show divergence is an appointment metric',
  ),
  'SW-088': unsup(
  ...
  'SW-092': other(
    ...
    'CRM completeness is a CRM-hygiene metric',
  ),
  'SW-093': other(
  ...
  'SW-095': other('CRM audit log', 'mass status changes is a CRM-audit metric'),
  'SW-096': other(
    'CRM closed-lost notes',
    'empty-notes is a CRM-hygiene metric',
  ),
  // §13 Compliance & Risk (Sales boundary)
  'SW-097': bound('Do-Not-Call/Do-Not-Contact is a compliance condition'),
  'SW-098': bound('TCPA consent is a compliance condition'),
  'SW-099': bound('adverse-action notice is a compliance condition'),
  'SW-100': bound('OFAC/Red-Flags is a compliance condition'),
  'SW-101': bound('driver-license-on-file is a compliance condition'),
  'SW-102': bound('privacy-policy consent is a compliance condition'),
  'SW-103': bound('underage/ID-mismatch is a compliance condition'),
  'SW-104': bound('Safeguards-Rule access log is a compliance condition'),
  // §14 Team / Managerial Dynamics
  'SW-105': other(
    'lead assignment distribution',
    'premium-lead fairness is a routing metric',
  ),
  'SW-106': other('lead reassignment', 'reassignment rate is a routing metric'),
  'SW-107': other('desking overrides', 'manager overrides is a desking metric'),
  'SW-108': hist(
  ...
  'SW-110': other(
    ...
    'poaching is a CRM ownership metric',
  ),
  ...
  'SW-113': other(
    'set + show composite',
    'set-vs-show is an appointment metric',
  ),
  'SW-114': other(
    'show + close composite',
    'show-vs-close needs deal outcomes',
  ),
  ...
  'SW-117': other(
    'BDC set + floor close',
    'handoff friction needs appointment + deal',
  ),
  'SW-118': bound(
  ...
  'SW-120': other(
    ...
    'competitor launch is external market data',
  ),
  // §16 Anomaly & Statistical Triggers
  'SW-121': hist(
    'trailing-30-day KPI mean',
    '2σ move needs the KPI series/baseline',
  ),
  ...
  'SW-124': hist(
    'cohort baseline',
    'cohort divergence needs prior-cohort history',
  ),
  'SW-125': other(
    'multi-KPI correlation',
    'decoupling needs multiple KPI families',
  ),
  'SW-126': other(
    'segment KPI slices',
    'segment anomaly needs multi-family KPIs',
  ),
  'SW-127': other(
  ...
  'SW-128': other(
    'alert dedup engine',
    'duplicate suppression is an engine concern',
  ),
  'SW-129': other(
    'alert context bundling',
    'context bundling is an engine concern',
  ),
  'SW-130': other('alert feedback store', 'feedback loop is an engine concern'),
  'SW-131': nlp(
    'NLP over notes/messages',
    'semantic layer needs message/notes content NLP',
  ),
  // PART 1 — Red Flags (Response/Cadence structural; else content)
  'SW-132': pend(
    ['thread_token', 'direction', 'activity_iso'],
  ...
  'SW-139': nlp(
    'stated-callback-time extraction',
    '"call me after 5" is content',
  ),
  'SW-140': unsup(
  ...
  'SW-143': nlp(
    ...
    "reply-doesn't-answer needs content",
  ),
  'SW-144': nlp('info-reuse semantics', 'ignoring provided info needs content'),
  'SW-145': nlp(
    'message-body similarity',
    'same-body-to-many needs content comparison',
  ),
  'SW-146': nlp('question detection', 'zero-questions needs content'),
  'SW-147': nlp('VOI mention', 'mentioning the vehicle by name needs content'),
  ...
  'SW-152': nlp(
    ...
    'unaddressed price concern needs content',
  ),
  'SW-153': nlp(
  ...
  'SW-157': nlp(
    'answer-vs-evasion semantics',
    'evasion pattern needs message meaning',
  ),
  'SW-158': nlp('credit-challenge mention', 'credit challenge needs content'),
  'SW-159': nlp('urgency phrases', 'urgency needs content'),
  'SW-160': nlp(
    'pre-objection discount',
    'offering a discount early needs content',
  ),
  'SW-161': nlp('competitor mention', 'competitor named needs content'),
  'SW-162': nlp('shopping-around phrases', 'comparison-shopping needs content'),
  ...
  'SW-166': nlp('tone-shift detection', 'short-reply tone shift needs content'),
  'SW-167': hist(
  ...
  'SW-174': nlp(
    ...
    '"frustrated/disappointed" needs content',
  ),
  ...
  'SW-177': nlp('manager-request phrase', '"speak to a manager" needs content'),
  ...
  'SW-185': nlp('repeat-ask semantics', '"asks same info twice" needs content'),
  'SW-186': bound('outbound SMS TCPA consent is a compliance condition'),
  'SW-187': bound('message to a DNC number is a compliance condition'),
  'SW-188': bound('legal contact-hours is a compliance condition'),
  'SW-189': bound('APR/terms disclosure is a compliance condition'),
  'SW-190': bound('discriminatory-language flag is a compliance condition'),
  'SW-191': bound(
    'SSN/DOB request via unsecured channel is a compliance condition',
  ),
  'SW-192': bound('plain-text PII in message body is a compliance condition'),
  'SW-193': nlp(
  ...
  'SW-199': bound('Service advisor coordination is Service-domain'),
  'SW-200': nlp(
    'stated-vehicle-interest extraction',
    'interest changes need content',
  ),
  'SW-201': nlp('need-confirmation semantics', 'confirming need needs content'),
  'SW-202': nlp(
    'language detection',
    'Spanish/other-language mismatch needs message language',
  ),
  'SW-203': nlp('reading-level analysis', 'reading-level match needs content'),
  'SW-204': nlp(
    'competence-gap semantics',
    'feature/model knowledge needs content',
  ),
  'SW-205': nlp(
    'emotional escalation curve',
    'sentiment-worsening needs content',
  ),
  'SW-206': nlp(
    'question repetition semantics',
    'repeated question needs content',
  ),
  ...
  'SW-211': other(
    'trade-valuation history',
    'declined-on-trade needs trade + history',
  ),
  'SW-212': other(
  ...
  'SW-215': other(
    'equity position',
    'positive-equity owners needs equity data',
  ),
  'SW-216': other(
    'payoff vs trade value',
    'payoff<trade needs finance/trade data',
  ),
  'SW-217': other('lease maturity', 'leases maturing needs lease data'),
  'SW-218': bound(
    'warranty-expiration CPO prospecting is classed outside the governed boundary',
  ),
  'SW-219': other(
    'OEM redesign/inventory',
    'model-redesign needs OEM/inventory data',
  ),
  'SW-220': other(
  ...
  'SW-221': other(
    'finance term tenure',
    '24+-months-financed needs finance data',
  ),
  'SW-222': bound(
    'multi-vehicle-household service is Service/cross-domain (outside governed boundary)',
  ),
  'SW-223': bound('Service ROs / declined work is Service-domain'),
  'SW-224': bound('vehicles in Service is Service-domain'),
  'SW-225': bound('repeat Service customers is Service-domain'),
  'SW-226': bound('Service customers / safety recalls is Service-domain'),
  'SW-227': bound('Service-advisor notes is Service-domain'),
  'SW-228': bound(
    'service waiting-room prospecting is Service-domain (outside governed boundary)',
  ),
  'SW-229': bound(
    'totaled/major-repair vehicle is Service-domain (outside governed boundary)',
  ),
  'SW-230': other('website VDP views', 'VDP retargeting is a website metric'),
  'SW-231': other(
    'payment-calculator usage',
    'calculator usage is a website metric',
  ),
  'SW-232': other(
    'trade-in tool completions',
    'trade tool is a website metric',
  ),
  'SW-233': unsup(
  ...
  'SW-234': unsup(
    ...
    'click tracking is not a derivative field',
  ),
  'SW-235': unsup(
  ...
  'SW-241': other(
    'review + referral',
    'positive-review referral needs review data',
  ),
  'SW-242': other(
    'sold purchase history',
    'repeat purchasers needs sold history',
  ),
  'SW-243': other(
    'OEM/affinity membership',
    'affinity groups needs OEM/affinity data',
  ),
  'SW-244': other(
  ...
  'SW-246': other(
    'occupation/employer data',
    'employer patterns needs external data',
  ),
  'SW-247': other(
  ...
  'SW-249': other(
    'trade conquest history',
    'cross-brand conquest needs trade history',
  ),
  'SW-250': other(
  ...
  'SW-259': other(
    'small-business/fleet data',
    'SMB owners needs external data',
  ),
  'SW-260': other(
    'retiree/downsizer data',
    'retirees needs external/segment data',
  ),
  'SW-261': hist(
  ...
  'SW-263': bound('seasonal Service customers is Service-domain'),
  'SW-264': bound(
    'tax-refund prospecting is classed outside the governed boundary',
  ),
  'SW-265': other(
    'lease-end dates',
  ...
  'SW-267': bound(
    'cross-rooftop introduction is outside the single-rooftop boundary',
  ),
  'SW-268': bound(
    'cross-rooftop trade/wish-list is outside the single-rooftop boundary',
  ),
  'SW-269': bound(
    'cross-brand switching is classed outside the governed boundary',
  ),
  'SW-270': bound('service-customer cross-marketing is Service-domain'),
  'SW-271': bound('shared DNC/DNS list is a compliance/cross-rooftop concern'),
  'SW-272': bound(
    'appended home-purchase records is an external/boundary data source',
  ),
  'SW-273': bound(
    'credit-tier refresh is classed outside the governed boundary',
  ),
  'SW-274': bound('vehicle-registration data is an external/boundary source'),
  'SW-275': bound('insurance-total-loss feed is an external/boundary source'),
  'SW-276': bound('public LinkedIn signals is an external/boundary source'),
  'SW-277': other(
  ...
  'SW-285': nlp(
    'intent classifier',
    'buying/objection intent needs content NLP',
  ),
