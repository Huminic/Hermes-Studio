#!/usr/bin/env python3
"""PKT-07-01 J1 freeze-candidate binding validator (compact, non-recursive).

All nine targets are held source_investigation_pending. Communication corpus supporting-only (never
promoted; lacks stable IDs; labels/keywords are not identifiers/findings/linkage); conditional
presently-not-proved wording; all content-dependent -> protected-content (no content/raw quotes/PII/
IDs/offer details). Provisional language patterns and sensitive life events are hypotheses, NOT
confirmed intent; causality must not be asserted. SW-169 extra sensitivity: no medical/employment/
moving/life-event detail retained/exposed/targeted/treated as verified; only a separately approved
privacy-safe aggregate rule under Duane sensitive-content authority.

Usage:
  python3 scripts/halo-phase1b/validate_pkt_07_01_binding.py            # writes CHECKS
  python3 scripts/halo-phase1b/validate_pkt_07_01_binding.py --no-write # validate only
"""
...
BASELINE_COMMIT = "5834c5e7dcc6bf6c03f3877fb2b68d0526c37840"
BINDING_REL = "docs/halo/contract/phase1b/pkt-07-01-binding.json"
VALIDATOR_REL = "scripts/halo-phase1b/validate_pkt_07_01_binding.py"
CHECKS_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-07-01/PKT-07-01_BINDING_CHECKS.json"
TWO_DELTA_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-07-01/PKT-07-01_J1_TWO_DELTA.md"
ROADMAP_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-07-01/PKT-07-01_J1_internal_coverage_roadmap.md"
ALLOWLIST = [BINDING_REL, VALIDATOR_REL, CHECKS_REL, TWO_DELTA_REL, ROADMAP_REL]

IDS = ["SW-161","SW-162","SW-163","SW-164","SW-165","SW-166","SW-167","SW-168","SW-169"]
ACCEPTED_IDS = []
SIP_IDS = list(IDS)
SUPPORTING_IDS = list(IDS)
CONDITIONAL_SOURCE_IDS = list(IDS)
PROTECTED_FUTURE_IDS = list(IDS)
SENSITIVE_ID = "SW-169"
...
HELD_MARKERS = {
    "SW-161": ["presently","supporting","competitor"], "SW-162": ["presently","supporting","shopping"],
    "SW-163": ["presently","supporting","exit"], "SW-164": ["presently","supporting","offer"],
    "SW-165": ["presently","supporting","comparison"], "SW-166": ["presently","supporting","tone"],
    "SW-167": ["presently","supporting","silence"], "SW-168": ["presently","supporting","pause"],
    "SW-169": ["presently","supporting","life-event"],
}
CAUSAL_TOKEN = {"SW-165": "comparison-shopping", "SW-167": "disengagement",
                "SW-168": "purchase-delay", "SW-169": "life-event causation"}
...
def run_structural(b, ctx):
    ...
    if lp.get("accepted_measured_ids") != [] or lp.get("accepted_disposition_only_ids") != [] \
        ...
        e.append("PARTITION: only source_investigation_pending may be non-empty")
    ...
    if sorted(aa.get("ids_checked", [])) != sorted(SIP_IDS):
        e.append("AUTHORITY_ABSENCE: ids_checked != the nine held")
    ...
    for mid in IDS:
        ...
        if mid in SUPPORTING_IDS:
            ...
            # hypothesis / not-confirmed-intent discipline (all held)
            if "hypothesis" not in joined:
                e.append("INTENT %s: provisional pattern must be framed as a hypothesis, not confirmed intent" % mid)
        ...
        if mid == SENSITIVE_ID:
            for need in ("sensitive", "privacy-safe", "aggregate", "retained"):
                if need not in joined:
                    e.append("SENSITIVE %s: life-event condition must state '%s' (no detail retained; privacy-safe aggregate only)" % (mid, need))
            ...
            for k, v in m.items():
                ...
                for s in _strings(v):
                    ...
                    if tok in sl and any(a in sl for a in ASSERTIVE) and not any(n in sl for n in NEGATION):
                        e.append("CAUSAL %s: label '%s' asserted as fact: %r" % (mid, tok, s))
...
def _probe_defs():
    ...
    def accepted_bucket(b): b["lifecycle_partition"]["accepted_measured_ids"] = ["SW-161"]
    def calc_bucket(b): b["lifecycle_partition"]["calculation_pending_ids"] = ["SW-161"]
    def eval_18(b): b["accepted_measured_authority"]["authoritative_evaluated_total"] = 18
    def authority_claim(b): b["authority_absence_assertion"]["in_gate5b_evaluated"] = ["SW-161"]
    def slices_break(b):
        for k in b["source_family_slices"]:
            if b["source_family_slices"][k]:
                b["source_family_slices"][k] = b["source_family_slices"][k][:-1]; break
    _PROT = "content not read protected-content envelope stable identities minimization"
    _BASE = "the corpus is known and held as supporting context; labels are not stable linkage; presently unproved; read-only vinsolutions check; external if needed; no proxy; " + _PROT + "; hypothesis"
    def held_promote(b): _set("SW-161", "disposition", "measured_validated")(b)
    def erase_supporting(b): _set("SW-162", "missing_or_quarantine_evidence",
        ["the rule is undefined; presently unproved; read-only vinsolutions check; external if needed; no proxy; " + _PROT + "; hypothesis"])(b)
    def falsely_promote_supporting(b): _set("SW-163", "source_existence_state", "acquired_local")(b)
    def absolute_source_absence(b): _set("SW-164", "missing_or_quarantine_evidence",
    ...
    def protected_without_authority(b): _set("SW-161", "missing_or_quarantine_evidence",
        ["the corpus is known and held as supporting context; labels are not stable linkage; presently unproved; read-only vinsolutions check; external if needed; no proxy; competitor; hypothesis"])(b)
    def proxy_join(b): _set("SW-162", "no_proxy_or_inference", False)(b)
    def customer_label_as_identity(b): _set("SW-166", "missing_or_quarantine_evidence",
        ["labels are stable identity; read-only vinsolutions check; external if needed; no proxy"])(b)
    def confirmed_intent(b): _set("SW-161", "missing_or_quarantine_evidence",
        ["the corpus is known and held as supporting context; labels are not stable linkage; presently unproved; read-only vinsolutions check; external if needed; no proxy; " + _PROT + "; the competitor mention confirms churn intent"])(b)
    def sensitive_detail_retained(b): _set("SW-169", "missing_or_quarantine_evidence",
        ["the corpus is known and held as supporting context; labels are not stable linkage; presently unproved; read-only vinsolutions check; external if needed; no proxy; " + _PROT + "; the life-event delays purchase; hypothesis"])(b)
    def held_value(b): _set("SW-161", "value", 0)(b)
    def held_formula(b): _set("SW-162", "formula", "x/y")(b)
    def held_threshold(b): _set("SW-167", "threshold", 7)(b)
    def held_grade(b): _set("SW-163", "grade_value_or_range", "healthy")(b)
    def held_detection(b): _set("SW-169", "detection_rule", "fire")(b)
    def held_display(b): _set("SW-161", "future_display_eligibility", True)(b)
    def held_acquire(b): _set("SW-161", "direct_source_fields", ["x"])(b)
    def held_missing_zero(b): _set("SW-162", "missing_is_not_zero", False)(b)
    def held_authoritative(b): _set("SW-164", "authoritative", True)(b)
    def held_report_state(b): _set("SW-165", "report_acceptance_state", "accepted")(b)
    def held_recomputed(b): _set("SW-169", "recomputed_this_tranche", True)(b)
    def held_next_owner_mismatch(b): _set("SW-161", "next_action_owner", "Someone else")(b)
    def held_next_owner_not_accountable(b):
        for m in b["metrics"]:
            if m["id"] == "SW-162":
                m["next_action_owner"] = "Ghost"; m["immediate_action"]["owner"] = "Ghost"
    def quarantine_use(b): _set("SW-165", "source_family_intent", "lead_source_roi")(b)
    def marker_strip(b): _set("SW-163", "missing_or_quarantine_evidence", ["something vague"])(b)
    def causal_fact(b): _set("SW-167", "business_question", "confirmed disengagement present")(b)
    def held_vocab(b): _set("SW-162", "source_existence_state", "acquired_local")(b)
    def held_forbidden_key(b): _set("SW-163", "current_truth_ref", "x")(b)
    ...
    struct = [
        ...
        ("confirmed_intent", confirmed_intent), ("sensitive_detail_retained", sensitive_detail_retained),
...
def _legacy_probe_defs(legacy_now):
    ...
    return [
        ...
        ("legacy_new_error_added", base + ["ledger SW-161: source SRC-x not registered"]),
...
def main():
    ...
    receipt = {
        "artifact": "PKT-07-01_BINDING_CHECKS", "validator": VALIDATOR_REL,
        ...
        "checks": {
            ...
            "accepted_ids": ACCEPTED_IDS, "source_investigation_pending_ids": SIP_IDS,
            "supporting_ids": SUPPORTING_IDS, "conditional_source_ids": CONDITIONAL_SOURCE_IDS,
            "protected_future_ids": PROTECTED_FUTURE_IDS, "sensitive_id": SENSITIVE_ID,
