#!/usr/bin/env python3
"""PKT-06-03 J1 freeze-candidate binding validator (compact, non-recursive).

All twelve targets are held source_investigation_pending. Communication corpus supporting-only (never
promoted; lacks stable Communication/Lead/Global-Customer/native-thread IDs; labels are not
identifiers/findings/linkage); conditional presently-not-proved wording; all content-dependent ->
protected-content (no content read). Interpretation labels (didn't read / unclear discovery / dodging
/ emotion / intent / alignment / personalization) are unproved labels, not facts. SW-158: no missing-
handoff inference without a governed event/source. SW-200: no poor-discovery inference from changing
interest without a rule and a stable chronology.

Usage:
  python3 scripts/halo-phase1b/validate_pkt_06_03_binding.py            # writes CHECKS
  python3 scripts/halo-phase1b/validate_pkt_06_03_binding.py --no-write # validate only
"""
...
BASELINE_COMMIT = "76ddfde6ffc52233b04155409f22e67c6242f5dd"
BINDING_REL = "docs/halo/contract/phase1b/pkt-06-03-binding.json"
VALIDATOR_REL = "scripts/halo-phase1b/validate_pkt_06_03_binding.py"
CHECKS_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-06-03/PKT-06-03_BINDING_CHECKS.json"
TWO_DELTA_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-06-03/PKT-06-03_J1_TWO_DELTA.md"
ROADMAP_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-06-03/PKT-06-03_J1_internal_coverage_roadmap.md"
ALLOWLIST = [BINDING_REL, VALIDATOR_REL, CHECKS_REL, TWO_DELTA_REL, ROADMAP_REL]

IDS = ["SW-158","SW-159","SW-185","SW-200","SW-201","SW-202","SW-203","SW-205","SW-206","SW-285","SW-287","SW-289"]
ACCEPTED_IDS = []
SIP_IDS = list(IDS)
...
HELD_MARKERS = {
    "SW-158": ["presently","supporting","handoff"], "SW-159": ["presently","supporting","urgency"],
    "SW-185": ["presently","supporting","repeated"], "SW-200": ["presently","supporting","changing"],
    "SW-201": ["presently","supporting","need"], "SW-202": ["presently","supporting","language"],
    "SW-203": ["presently","supporting","reading"], "SW-205": ["presently","supporting","emotional"],
    "SW-206": ["presently","supporting","dodging"], "SW-285": ["presently","supporting","intent"],
    "SW-287": ["presently","supporting","alignment"], "SW-289": ["presently","supporting","personalization"],
}
CAUSAL_TOKEN = {"SW-185": "didn't read", "SW-200": "unclear discovery", "SW-206": "dodging"}
...
def run_structural(b, ctx):
    ...
    if sorted(lp.get("source_investigation_pending_ids", [])) != sorted(SIP_IDS):
        e.append("PARTITION: source_investigation_pending_ids != the twelve held")
    ...
    if sorted(aa.get("ids_checked", [])) != sorted(SIP_IDS):
        e.append("AUTHORITY_ABSENCE: ids_checked != the twelve held")
    ...
    for mid in IDS:
        ...
        if mid == HANDOFF_ID:
            for need in ("handoff", "event"):
                if need not in joined:
                    e.append("HANDOFF %s: must forbid missing-handoff inference without a governed event/source ('%s')" % (mid, need))
        if mid == DISCOVERY_ID:
            for need in ("changing", "chronology"):
                if need not in joined:
                    e.append("DISCOVERY %s: must forbid poor-discovery inference from changing interest without rule/chronology ('%s')" % (mid, need))
...
def _probe_defs():
    ...
    def accepted_bucket(b): b["lifecycle_partition"]["accepted_measured_ids"] = ["SW-158"]
    def calc_bucket(b): b["lifecycle_partition"]["calculation_pending_ids"] = ["SW-158"]
    def eval_18(b): b["accepted_measured_authority"]["authoritative_evaluated_total"] = 18
    def authority_claim(b): b["authority_absence_assertion"]["in_gate5b_evaluated"] = ["SW-158"]
    def slices_break(b):
        for k in b["source_family_slices"]:
            if b["source_family_slices"][k]:
                b["source_family_slices"][k] = b["source_family_slices"][k][:-1]; break
    def held_promote(b): _set("SW-158", "disposition", "measured_validated")(b)
    def erase_supporting(b): _set("SW-159", "missing_or_quarantine_evidence",
    ...
    def falsely_promote_supporting(b): _set("SW-185", "source_existence_state", "acquired_local")(b)
    def absolute_source_absence(b): _set("SW-159", "missing_or_quarantine_evidence",
    ...
    def protected_without_authority(b): _set("SW-158", "missing_or_quarantine_evidence",
        ["the corpus is known and held as supporting context; labels are not stable linkage; presently unproved; read-only vinsolutions check; external if needed; no proxy; a governed handoff event is required"])(b)
    def proxy_join(b): _set("SW-185", "no_proxy_or_inference", False)(b)
    def customer_label_as_identity(b): _set("SW-201", "missing_or_quarantine_evidence",
        ["labels are stable identity; read-only vinsolutions check; external if needed; no proxy"])(b)
    def handoff_missing_inference(b): _set("SW-158", "missing_or_quarantine_evidence",
        ["the corpus is known and held as supporting context; labels are not stable linkage; presently unproved; read-only vinsolutions check; external if needed; no proxy; content not read protected-content envelope stable identities minimization; a missing handoff is present"])(b)
    def discovery_without_chronology(b): _set("SW-200", "missing_or_quarantine_evidence",
        ["the corpus is known and held as supporting context; labels are not stable linkage; presently unproved; read-only vinsolutions check; external if needed; no proxy; content not read protected-content envelope stable identities minimization; the changing interest shows poor discovery"])(b)
    def held_value(b): _set("SW-158", "value", 0)(b)
    def held_formula(b): _set("SW-159", "formula", "x/y")(b)
    def held_threshold(b): _set("SW-200", "threshold", 3)(b)
    def held_grade(b): _set("SW-185", "grade_value_or_range", "healthy")(b)
    def held_detection(b): _set("SW-289", "detection_rule", "fire")(b)
    def held_display(b): _set("SW-158", "future_display_eligibility", True)(b)
    def held_acquire(b): _set("SW-158", "direct_source_fields", ["x"])(b)
    def held_missing_zero(b): _set("SW-185", "missing_is_not_zero", False)(b)
    def held_authoritative(b): _set("SW-202", "authoritative", True)(b)
    def held_report_state(b): _set("SW-203", "report_acceptance_state", "accepted")(b)
    def held_recomputed(b): _set("SW-289", "recomputed_this_tranche", True)(b)
    def held_next_owner_mismatch(b): _set("SW-158", "next_action_owner", "Someone else")(b)
    def held_next_owner_not_accountable(b):
        for m in b["metrics"]:
            if m["id"] == "SW-159":
                m["next_action_owner"] = "Ghost"; m["immediate_action"]["owner"] = "Ghost"
    def quarantine_use(b): _set("SW-200", "source_family_intent", "lead_source_roi")(b)
    def marker_strip(b): _set("SW-185", "missing_or_quarantine_evidence", ["something vague"])(b)
    def causal_fact(b): _set("SW-206", "business_question", "confirmed dodging present")(b)
    def held_vocab(b): _set("SW-159", "source_existence_state", "acquired_local")(b)
    def held_forbidden_key(b): _set("SW-185", "current_truth_ref", "x")(b)
    ...
    struct = [
        ...
        ("handoff_missing_inference", handoff_missing_inference),
        ("discovery_without_chronology", discovery_without_chronology),
...
def _legacy_probe_defs(legacy_now):
    ...
    return [
        ...
        ("legacy_new_error_added", base + ["ledger SW-158: source SRC-x not registered"]),
...
def main():
    ...
    receipt = {
        "artifact": "PKT-06-03_BINDING_CHECKS", "validator": VALIDATOR_REL,
        ...
        "checks": {
            ...
            "protected_future_ids": PROTECTED_FUTURE_IDS, "handoff_id": HANDOFF_ID, "discovery_id": DISCOVERY_ID,
