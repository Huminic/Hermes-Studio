#!/usr/bin/env python3
"""PKT-04-01 J1 freeze-candidate binding validator (compact, non-recursive).

Validates docs/halo/contract/phase1b/pkt-04-01-binding.json against live repository
authorities at baseline f51ebb974, enforces the exact 5-file CREATE-only allowlist,
proves AUTHORITY-ABSENCE (none of the 11 targets appears in any evaluation authority,
so nothing may be valued/graded), runs the legacy validator and distinguishes the
exact 30 known errors from new, runs deterministic adversarial mutation probes, and
writes a deterministic CHECKS receipt. A second run with --no-write reproduces the
receipt byte-for-byte.

All 11 PKT-04-01 targets are held (none accepted/measured/graded): SW-049/050 are proven
Honda crm_sales_gross evidence (Report-1275) held as data_acquired_calculation_pending /
acquired_local / admitted_held (still not_measured), and the other nine are
source_investigation_pending. No value / formula / threshold / baseline / detection-rule
may be authored for any of them.

Usage:
  python3 scripts/halo-phase1b/validate_pkt_04_01_binding.py            # writes CHECKS
  python3 scripts/halo-phase1b/validate_pkt_04_01_binding.py --no-write # validate only
"""
...
BASELINE_COMMIT = "f51ebb974c010e92b945d2612111eecbd154299a"
BINDING_REL = "docs/halo/contract/phase1b/pkt-04-01-binding.json"
VALIDATOR_REL = "scripts/halo-phase1b/validate_pkt_04_01_binding.py"
CHECKS_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-04-01/PKT-04-01_BINDING_CHECKS.json"
TWO_DELTA_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-04-01/PKT-04-01_J1_TWO_DELTA.md"
ROADMAP_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-04-01/PKT-04-01_J1_internal_coverage_roadmap.md"
ALLOWLIST = [BINDING_REL, VALIDATOR_REL, CHECKS_REL, TWO_DELTA_REL, ROADMAP_REL]

IDS = ["SW-047","SW-048","SW-049","SW-050","SW-051","SW-052",
       "SW-053","SW-054","SW-055","SW-056","SW-057"]
ACCEPTED = []                       # PKT-04-01 introduces zero accepted metrics
HELD = [i for i in IDS if i not in ACCEPTED]
CALC_IDS = ["SW-049","SW-050"]      # proven crm_sales_gross evidence, calculation-pending
SIP_IDS = [i for i in IDS if i not in CALC_IDS]   # the other nine, source_investigation_pending
UNPROVEN_SOURCE_IDS = ["SW-051","SW-052","SW-053","SW-054","SW-055","SW-056"]
JOIN_IDS = ["SW-057"]
MANUAL_IDS = ["SW-047","SW-048"]

# SW-049/050 proven-evidence tokens; SW-057 ownership role strings + technical-join tokens;
# forbidden absolute source claims for SW-051..056.
CALC_EVID_TOKENS = ("native-scheduled-evidence", "Report-1275", "crm_sales_gross", "readCrmSalesGross")
CALC_STATE = {  # exact held-but-acquired lifecycle for the proven crm_sales_gross rows
...
JOIN_TECH_TOKENS = ("join design", "join definition", "define the join", "join implementation",
                    "implement the join", "cardinality", "source key", "technical join")
...
# legacy validate_phase1b.py canonical baseline (post-PKT-03-02-J2, verified at f51ebb974)
...
HELD_MARKERS = {
    "SW-047": ["manual","write-up","pencil"],
    "SW-048": ["manual","pencil","negotiation drag"],
    "SW-049": ["crm sales gross","report-1275","trailing"],
    "SW-050": ["crm sales gross","report-1275","denominator"],
    "SW-051": ["presently","proxy","penetration"],
    "SW-052": ["presently","proxy","pvr"],
    "SW-053": ["presently","proxy","cash"],
    "SW-054": ["presently","proxy","trade"],
    "SW-055": ["presently","proxy","stips"],
    "SW-056": ["presently","proxy","rebate"],
    "SW-057": ["join","aging","concentration"],
}
CAUSAL_TOKEN = {  # catalog causal/business labels that must not be asserted as fact
    "SW-048": "negotiation drag",
    "SW-053": "missed product opportunity",
    "SW-057": "pull-through",
}
...
# no accepted rows in PKT-04-01: these top-level fact keys must NOT appear on any metric
...
def run_structural(b, ctx):
    ...
    # --- lifecycle partition disjoint + covers 11, ALL held ---
    ...
    if lp.get("accepted_measured_ids") != []:
        e.append("PARTITION: accepted_measured_ids must be [] (no accepted metrics in PKT-04-01)")
    if lp.get("accepted_disposition_only_ids") != [] or lp.get("rejected_ids") != []:
        e.append("PARTITION: accepted_disposition_only_ids / rejected_ids must be empty")
    if sorted(lp.get("calculation_pending_ids", [])) != sorted(CALC_IDS):
        e.append("PARTITION: calculation_pending_ids != [SW-049, SW-050]")
    if sorted(lp.get("source_investigation_pending_ids", [])) != sorted(SIP_IDS):
        e.append("PARTITION: source_investigation_pending_ids != the nine held")

    # --- accepted authority block: zero accepted, evaluated stays 17 (live) ---
    ...
    # --- AUTHORITY-ABSENCE: none of the 11 in any evaluation authority (live), and the
    #     binding assertion must match the live computed intersections ---
    ...
    # --- source_family_slices sum to 11 unique ---
    ...
    for mid in IDS:
        ...
        # --- every target is HELD: nothing measured / valued / graded ---
        # Two held sub-classes: CALC (SW-049/050 proven crm_sales_gross, calculation-pending)
        # and SIP (the other nine, source_investigation_pending). Both stay not_measured/draft/no-value.
        if mid in CALC_IDS:
            if m.get("disposition") != CALC_STATE["disposition"]:
                e.append("CALC %s: disposition must be data_acquired_calculation_pending" % mid)
            if m.get("source_existence_state") != CALC_STATE["source_existence_state"]:
                e.append("CALC %s: source_existence_state must be acquired_local" % mid)
            if m.get("acquisition_admission_state") != CALC_STATE["acquisition_admission_state"]:
                e.append("CALC %s: acquisition_admission_state must be admitted_held" % mid)
        else:
        ...
        # no accepted-only fact keys may leak onto a held metric
        ...
        # source_family_intent must not be a quarantined family (no quarantined-source cure)
        if m.get("source_family_intent") in QUARANTINED:
            e.append("HELD %s: source_family_intent is quarantined family" % mid)

        # CALC (SW-049/050): proven crm_sales_gross admitted_held; must cite Report-1275 + E2E
        # evidence and carry a distinct held_reason; no value/grade (enforced above).
        if mid in CALC_IDS:
            if m.get("source_family_intent") != "crm_sales_gross":
                e.append("CALC %s: source_family_intent must be crm_sales_gross (proven family)" % mid)
            ref = str(m.get("held_source_evidence_ref", ""))
            for need in CALC_EVID_TOKENS:
                if need not in ref:
                    e.append("CALC %s: held_source_evidence_ref must cite '%s'" % (mid, need))
            if not str(m.get("held_reason", "")).strip():
                e.append("CALC %s: missing held_reason (why proven evidence stays calculation-pending)" % mid)

        # SW-051..056: conditional 'not presently proved/found' wording; reject absolute claims;
        # intent must NOT predetermine external/CRM; keep no proxy.
        if mid in UNPROVEN_SOURCE_IDS:
            if not str(m.get("source_family_intent", "")).endswith("_source_unproved"):
                e.append("HELD %s: unproved-source condition must use a *_source_unproved intent (no predetermined external/CRM)" % mid)
        ...
        # SW-057: the technical join must be stated (ownership split enforced below)
        if mid in JOIN_IDS and "join" not in joined:
            e.append("HELD %s: join condition must state the required join in evidence text" % mid)
        ...
        # SW-057 ownership split: Duane owns ONLY business meaning + thresholds; Codex owns the
        # source-key/cardinality read-only proof + acquisition; Studio owns the technical join
        # design + implementation. Reject any technical-join role/action assigned to Duane.
        if mid in JOIN_IDS:
            owners_map = m.get("accountable_owners", {}) or {}
            join_actions = []
            if isinstance(m.get("immediate_action"), dict):
                join_actions.append(m["immediate_action"])
            join_actions += [a for a in m.get("subsequent_actions", []) if isinstance(a, dict)]
            def _norm(s): return str(s).lower().replace("_", " ")
            for role, who in owners_map.items():
                if who and who.startswith("Duane") and any(t in _norm(role) for t in JOIN_TECH_TOKENS):
                    e.append("JOIN %s: Duane assigned a technical-join role: %s" % (mid, role))
            for a in join_actions:
                if a.get("owner", "").startswith("Duane") and any(t in _norm(a.get("action", "")) for t in JOIN_TECH_TOKENS):
                    e.append("JOIN %s: Duane assigned a technical-join action: %r" % (mid, a.get("action")))
            if not any(who == STUDIO_ROLE and "join" in _norm(role) and ("design" in _norm(role) or "implement" in _norm(role))
                       for role, who in owners_map.items()):
                e.append("JOIN %s: Studio must own the technical join design/implementation" % mid)
            if not any(who == CODEX_ROLE and ("cardinality" in _norm(role) or ("source" in _norm(role) and "key" in _norm(role)))
                       for role, who in owners_map.items()):
                e.append("JOIN %s: Codex must own the join source-key/cardinality proof + acquisition" % mid)
...
def legacy_delta_errors(now):
    """PURE: the legacy validator's result must remain EXACTLY the pinned 30-error signature."""
    e = []
    now_s = sorted(now)
...
def _probe_defs():
    ...
    def accepted_bucket(b): b["lifecycle_partition"]["accepted_measured_ids"] = ["SW-047"]
    def eval_18(b): b["accepted_measured_authority"]["authoritative_evaluated_total"] = 18
    def authority_claim_g2(b): b["authority_absence_assertion"]["in_gate2_evaluable_conditions"] = ["SW-047"]
    def slices_break(b):
        for k in b["source_family_slices"]:
            if b["source_family_slices"][k]:
                b["source_family_slices"][k] = b["source_family_slices"][k][:-1]; break
    def held_promote(b): _set("SW-047", "disposition", "measured_validated")(b)
    def held_value(b): _set("SW-049", "value", 0)(b)
    def held_formula(b): _set("SW-051", "formula", "x/y")(b)
    def held_threshold(b): _set("SW-052", "threshold", 1200)(b)
    def held_grade(b): _set("SW-050", "grade_value_or_range", "healthy")(b)
    def held_detection(b): _set("SW-057", "detection_rule", "fire when concentrated")(b)
    def held_baseline(b): _set("SW-049", "ot_anchor", {"threshold": 1})(b)
    def held_display(b): _set("SW-048", "future_display_eligibility", True)(b)
    def held_acquire(b): _set("SW-047", "direct_source_fields", ["write_up_ts"])(b)
    def held_missing_zero(b): _set("SW-049", "missing_is_not_zero", False)(b)
    def held_proxy(b): _set("SW-051", "no_proxy_or_inference", False)(b)
    def held_authoritative(b): _set("SW-053", "authoritative", True)(b)
    def held_report_state(b): _set("SW-054", "report_acceptance_state", "accepted")(b)
    def held_recomputed(b): _set("SW-055", "recomputed_this_tranche", True)(b)
    def held_next_owner_mismatch(b): _set("SW-047", "next_action_owner", "Someone else entirely")(b)
    def held_next_owner_not_accountable(b):
        for m in b["metrics"]:
            if m["id"] == "SW-048":
                m["next_action_owner"] = "Ghost owner"
                m["immediate_action"]["owner"] = "Ghost owner"
    def quarantine_use(b): _set("SW-057", "source_family_intent", "lead_source_roi")(b)
    def source_substitution(b): _set("SW-051", "source_family_intent", "crm_sales_gross")(b)
    def marker_strip(b): _set("SW-057", "missing_or_quarantine_evidence", ["something vague"])(b)
    def calc_marker_strip(b): _set("SW-049", "missing_or_quarantine_evidence",
                                   ["trailing history missing; no proxy"])(b)
    def causal_fact(b): _set("SW-048", "business_question", "confirmed negotiation drag present")(b)
    def duane_technical(b):
        for m in b["metrics"]:
            if m["id"] == "SW-047":
                m["immediate_action"] = {"owner": "Duane (x)", "action": "acquire the timestamp series"}
    def held_vocab(b): _set("SW-051", "source_existence_state", "acquired_local")(b)
    def held_forbidden_key(b): _set("SW-052", "peer_rank", {"rank": 1})(b)

    # --- three requested regression probes for this shadow correction ---
    # (1) regression of the SW-049/050 known proven-held-source state
    def calc_regression(b):
        for m in b["metrics"]:
            if m["id"] == "SW-049":
                m["disposition"] = "source_investigation_pending"
                m["source_existence_state"] = "unproved"
                m["acquisition_admission_state"] = "not_acquired"
    # (2) absolute VinSolutions-export-absence / external-required claim for SW-051..056
    def absolute_vin_absence(b): _set("SW-052", "missing_or_quarantine_evidence",
        ["no supporting VinSolutions export exists for this condition; an external source is required"])(b)
    # (3) technical join ownership assigned to Duane on SW-057
    def duane_join_technical(b):
        for m in b["metrics"]:
            if m["id"] == "SW-057":
                m["immediate_action"] = {"owner": "Duane (x)", "action": "define the join and prove source key cardinality"}
    ...
    struct = [
        ...
        ("quarantined_family_use", quarantine_use), ("source_substitution", source_substitution),
        ("held_blocker_marker_strip", marker_strip), ("calc_marker_strip", calc_marker_strip),
        ("causal_diagnosis_fact", causal_fact), ("duane_technical_action", duane_technical),
        ("held_vocab_inconsistency", held_vocab), ("held_forbidden_accepted_key", held_forbidden_key),
        ("calc_regression_known_held_source", calc_regression),
        ("absolute_vin_export_absence_claim", absolute_vin_absence),
        ("technical_join_owner_duane", duane_join_technical),
    ]
    for n, f in struct:
        add(n, f)
    return probes
...
def _legacy_probe_defs(legacy_now):
    """Legacy-delta mutation probes: each synthetic legacy error list MUST be rejected."""
    if legacy_now is None:
        return []
    base = sorted(legacy_now)
    return [
        ("legacy_same_count_substitution", base[:-1] + ["ledger SW-999: fabricated swap error"]),
        ("legacy_new_error_added", base + ["ledger SW-047: source SRC-x not registered"]),
...
def main():
    ...
    # catalog pin (explicit, in addition to the pinned-hash sweep)
    if ctx and sha256("docs/halo/contract/semantic-watchdog-feasibility-matrix-295.json") != CATALOG_SHA_EXPECTED:
    ...
    # (b) self-check: the absent-at-baseline predicate must reject a known baseline-existing path
    ...
    # changed-set vs baseline: union of (baseline<->worktree tracked diff) and untracked, .claude ignored.
    ...
    # (c) reject any .claude staging and any staged path outside the allowlist
    ...
    # (d) prior packets preserved byte-identical vs baseline
    preserve_ok = True
    for f in PRESERVE:
    ...
    # legacy validator: exactly 30 known errors, 0 new (J1 changes no ledger/gate authority)
    legacy_now = None
    ...
    # adversarial probes (structural + legacy-delta)
    probe_results = []
    probe_fail = 0
    ...
    receipt = {
        "artifact": "PKT-04-01_BINDING_CHECKS",
        ...
        "checks": {
            ...
            "calculation_pending_ids": CALC_IDS,
