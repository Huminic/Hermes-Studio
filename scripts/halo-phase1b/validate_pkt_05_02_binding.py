#!/usr/bin/env python3
"""PKT-05-02 J1 freeze-candidate binding validator (compact, non-recursive).

All nine targets are held source_investigation_pending (none accepted/measured/graded). Every
unproved source uses conditional 'not presently proved/found' wording (never nonexistent, never
predetermined external). Known component/corpus/current-state evidence is supporting-only (never
erased, never promoted). The CAGE aggregate is supporting-only and quarantined (no cure; does not
prove daily decline). Current status/last-edited never substitutes for transition history
(SW-106/107/110). SW-029/030 read no content and require a future protected-content envelope.

Usage:
  python3 scripts/halo-phase1b/validate_pkt_05_02_binding.py            # writes CHECKS
  python3 scripts/halo-phase1b/validate_pkt_05_02_binding.py --no-write # validate only
"""
...
BASELINE_COMMIT = "8506c0d95ace686ab8474ebaef724a01de684f31"
BINDING_REL = "docs/halo/contract/phase1b/pkt-05-02-binding.json"
VALIDATOR_REL = "scripts/halo-phase1b/validate_pkt_05_02_binding.py"
CHECKS_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-05-02/PKT-05-02_BINDING_CHECKS.json"
TWO_DELTA_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-05-02/PKT-05-02_J1_TWO_DELTA.md"
ROADMAP_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-05-02/PKT-05-02_J1_internal_coverage_roadmap.md"
ALLOWLIST = [BINDING_REL, VALIDATOR_REL, CHECKS_REL, TWO_DELTA_REL, ROADMAP_REL]

IDS = ["SW-028","SW-029","SW-030","SW-105","SW-106","SW-107","SW-108","SW-109","SW-110"]
ACCEPTED_IDS = []
SIP_IDS = list(IDS)
SUPPORTING_IDS = ["SW-029","SW-105","SW-106","SW-107","SW-109","SW-110"]
CONDITIONAL_SOURCE_IDS = list(IDS)          # every unproved source uses conditional wording
PROTECTED_FUTURE_IDS = ["SW-029","SW-030"]
MUST_NOT_SUBSTITUTE_IDS = ["SW-106","SW-107","SW-110"]
CAGE_ID = "SW-109"
...
HELD_MARKERS = {
    "SW-028": ["presently","proxy","email"],
    "SW-029": ["presently","supporting","sentiment"],
    "SW-030": ["presently","proxy","complaint"],
    "SW-105": ["presently","supporting","premium"],
    "SW-106": ["presently","supporting","reassignment"],
    "SW-107": ["presently","supporting","override"],
    "SW-108": ["presently","proxy","cohort"],
    "SW-109": ["presently","supporting","cage"],
    "SW-110": ["presently","supporting","ownership"],
}
CAUSAL_TOKEN = {
    "SW-029": "pushy", "SW-105": "skimming", "SW-106": "routing dysfunction",
    "SW-109": "disengagement", "SW-110": "poaching",
}
...
def run_structural(b, ctx):
    ...
    for mid in IDS:
        ...
        if mid == CAGE_ID:
            for need in ("cage", "aggregate", "daily", "quarantin"):
                if need not in joined:
                    e.append("CAGE %s: CAGE evidence must state '%s' (supporting-only, quarantined, not daily)" % (mid, need))
...
def _probe_defs():
    ...
    def accepted_bucket(b): b["lifecycle_partition"]["accepted_measured_ids"] = ["SW-028"]
    def calc_bucket(b): b["lifecycle_partition"]["calculation_pending_ids"] = ["SW-028"]
    def eval_18(b): b["accepted_measured_authority"]["authoritative_evaluated_total"] = 18
    def authority_claim(b): b["authority_absence_assertion"]["in_gate5b_evaluated"] = ["SW-028"]
    def slices_break(b):
        for k in b["source_family_slices"]:
            if b["source_family_slices"][k]:
                b["source_family_slices"][k] = b["source_family_slices"][k][:-1]; break
    def held_promote(b): _set("SW-028", "disposition", "measured_validated")(b)
    def erase_supporting(b): _set("SW-105", "missing_or_quarantine_evidence", ["the rule is undefined; presently unproved; read-only vinsolutions check; external if needed; no proxy"])(b)
    def falsely_promote_supporting(b): _set("SW-109", "source_existence_state", "acquired_local")(b)
    def absolute_source_absence(b): _set("SW-028", "missing_or_quarantine_evidence",
        ["no email export exists for this condition; an external source is required"])(b)
    def technical_owner_duane(b):
        for m in b["metrics"]:
            if m["id"] == "SW-105":
                m["immediate_action"] = {"owner": "Duane (x)", "action": "define the join and prove the stable-key cardinality"}
    def protected_without_authority(b): _set("SW-029", "missing_or_quarantine_evidence",
    ...
    def proxy_join(b): _set("SW-105", "no_proxy_or_inference", False)(b)
    def cage_quarantine_cure(b): _set("SW-109", "source_family_intent", "cage_kpi")(b)
    def cage_daily_claim(b): _set("SW-109", "missing_or_quarantine_evidence",
        ["presently unproved; read-only vinsolutions check; external if needed; no proxy; supporting context known"])(b)
    def current_state_substitution(b): _set("SW-106", "missing_or_quarantine_evidence",
        ["presently unproved; read-only vinsolutions check; external if needed; supporting context known; no proxy"])(b)
    def held_value(b): _set("SW-028", "value", 0)(b)
    def held_formula(b): _set("SW-105", "formula", "x/y")(b)
    def held_threshold(b): _set("SW-108", "threshold", 30)(b)
    def held_grade(b): _set("SW-107", "grade_value_or_range", "healthy")(b)
    def held_detection(b): _set("SW-110", "detection_rule", "fire")(b)
    def held_display(b): _set("SW-029", "future_display_eligibility", True)(b)
    def held_acquire(b): _set("SW-028", "direct_source_fields", ["x"])(b)
    def held_missing_zero(b): _set("SW-105", "missing_is_not_zero", False)(b)
    def held_authoritative(b): _set("SW-030", "authoritative", True)(b)
    def held_report_state(b): _set("SW-108", "report_acceptance_state", "accepted")(b)
    def held_recomputed(b): _set("SW-110", "recomputed_this_tranche", True)(b)
    def held_next_owner_mismatch(b): _set("SW-028", "next_action_owner", "Someone else")(b)
    def held_next_owner_not_accountable(b):
        for m in b["metrics"]:
            if m["id"] == "SW-030":
                m["next_action_owner"] = "Ghost"; m["immediate_action"]["owner"] = "Ghost"
    def quarantine_use(b): _set("SW-105", "source_family_intent", "lead_source_roi")(b)
    def marker_strip(b): _set("SW-108", "missing_or_quarantine_evidence", ["something vague"])(b)
    def causal_fact(b): _set("SW-105", "business_question", "confirmed skimming present")(b)
    def held_vocab(b): _set("SW-028", "source_existence_state", "acquired_local")(b)
    def held_forbidden_key(b): _set("SW-108", "peer_rank", {"rank": 1})(b)
    ...
    struct = [
        ...
        ("cage_daily_history_claim", cage_daily_claim),
...
def _legacy_probe_defs(legacy_now):
    ...
    return [
        ...
        ("legacy_new_error_added", base + ["ledger SW-028: source SRC-x not registered"]),
...
def main():
    ...
    receipt = {
        "artifact": "PKT-05-02_BINDING_CHECKS", "validator": VALIDATOR_REL,
