#!/usr/bin/env python3
"""PKT-06-02 J1 freeze-candidate binding validator (compact, non-recursive).

Three accepted carry-forward (SW-145/149/150, deep-equal to live Gate 5B; only authority). Nine held
source_investigation_pending: communication corpus supporting-only (never promoted; labels are not
identifiers/findings/linkage); conditional presently-not-proved wording; all content-dependent ->
protected-content (no content read). No discovery / low effort / ignores / evasion (provisional
NLP/rule labels) are not facts.

Usage:
  python3 scripts/halo-phase1b/validate_pkt_06_02_binding.py            # writes CHECKS
  python3 scripts/halo-phase1b/validate_pkt_06_02_binding.py --no-write # validate only
"""
...
BASELINE_COMMIT = "63bd4c0bd087c98ae641663f494f5e174eee742b"
BINDING_REL = "docs/halo/contract/phase1b/pkt-06-02-binding.json"
VALIDATOR_REL = "scripts/halo-phase1b/validate_pkt_06_02_binding.py"
CHECKS_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-06-02/PKT-06-02_BINDING_CHECKS.json"
TWO_DELTA_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-06-02/PKT-06-02_J1_TWO_DELTA.md"
ROADMAP_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-06-02/PKT-06-02_J1_internal_coverage_roadmap.md"
ALLOWLIST = [BINDING_REL, VALIDATOR_REL, CHECKS_REL, TWO_DELTA_REL, ROADMAP_REL]

IDS = ["SW-145","SW-146","SW-147","SW-148","SW-149","SW-150","SW-151","SW-152","SW-153","SW-155","SW-156","SW-157"]
ACCEPTED_IDS = ["SW-145","SW-149","SW-150"]
...
HELD_MARKERS = {
    "SW-146": ["presently","supporting","discovery"], "SW-147": ["presently","supporting","vehicle"],
    "SW-148": ["presently","supporting","cta"], "SW-151": ["presently","supporting","greeting"],
    "SW-152": ["presently","supporting","price"], "SW-153": ["presently","supporting","financing"],
    "SW-155": ["presently","supporting","trade"], "SW-156": ["presently","supporting","out-the-door"],
    "SW-157": ["presently","supporting","evasion"],
}
CAUSAL_TOKEN = {"SW-146": "discovery", "SW-157": "evasion"}
...
def run_structural(b, ctx):
    ...
    if lp.get("accepted_measured_ids") != ACCEPTED_IDS:
        e.append("PARTITION: accepted_measured_ids != [SW-145, SW-149, SW-150]")
    ...
    if ama.get("ids") != ACCEPTED_IDS:
        e.append("ACCEPTED: accepted_measured_authority.ids != [SW-145, SW-149, SW-150]")
...
def _probe_defs():
    ...
    def cond_drift(b): b["metrics"][1]["canonical_condition"] = "TAMPERED"
    ...
    def accepted_bucket_wrong(b): b["lifecycle_partition"]["accepted_measured_ids"] = ["SW-146"]
    def calc_bucket(b): b["lifecycle_partition"]["calculation_pending_ids"] = ["SW-146"]
    def eval_18(b): b["accepted_measured_authority"]["authoritative_evaluated_total"] = 18
    def authority_claim(b): b["authority_absence_assertion"]["in_gate5b_evaluated"] = ["SW-146"]
    def slices_break(b):
        for k in b["source_family_slices"]:
            if b["source_family_slices"][k]:
                b["source_family_slices"][k] = b["source_family_slices"][k][:-1]; break
    def accepted_value_change(b): _set("SW-145", "value", 0.5)(b)
    def accepted_recompute(b): _set("SW-149", "recomputed_this_tranche", True)(b)
    def accepted_eval_tamper(b): _set("SW-150", "accepted_evaluation", {"tampered": True})(b)
    def accepted_false_anchor(b): _set("SW-145", "gate2_anchor", {"x": 1})(b)
    def accepted_legacy_surface(b): _set("SW-149", "peer_rank", {"rank": 1})(b)
    def accepted_demote(b): _set("SW-150", "disposition", "source_investigation_pending")(b)
    def accepted_condition_drift(b): _set("SW-145", "canonical_condition", "Same message body (no threshold).")(b)
    def accepted_threshold_replace(b): _set("SW-149", "threshold", 5)(b)
    def held_promote(b): _set("SW-146", "disposition", "measured_validated")(b)
    def erase_supporting(b): _set("SW-147", "missing_or_quarantine_evidence",
    ...
    def falsely_promote_supporting(b): _set("SW-147", "source_existence_state", "acquired_local")(b)
    def absolute_source_absence(b): _set("SW-148", "missing_or_quarantine_evidence",
    ...
    def protected_without_authority(b): _set("SW-146", "missing_or_quarantine_evidence",
        ["the corpus is known and held as supporting context; labels are not stable linkage; presently unproved; read-only vinsolutions check; external if needed; no proxy"])(b)
    def proxy_join(b): _set("SW-147", "no_proxy_or_inference", False)(b)
    def customer_label_as_identity(b): _set("SW-151", "missing_or_quarantine_evidence",
        ["labels are stable identity; read-only vinsolutions check; external if needed; no proxy"])(b)
    def held_value(b): _set("SW-146", "value", 0)(b)
    def held_formula(b): _set("SW-148", "formula", "x/y")(b)
    def held_threshold(b): _set("SW-156", "threshold", 2)(b)
    def held_grade(b): _set("SW-152", "grade_value_or_range", "healthy")(b)
    def held_detection(b): _set("SW-157", "detection_rule", "fire")(b)
    def held_display(b): _set("SW-146", "future_display_eligibility", True)(b)
    def held_acquire(b): _set("SW-146", "direct_source_fields", ["x"])(b)
    def held_missing_zero(b): _set("SW-147", "missing_is_not_zero", False)(b)
    def held_authoritative(b): _set("SW-153", "authoritative", True)(b)
    def held_report_state(b): _set("SW-155", "report_acceptance_state", "accepted")(b)
    def held_recomputed(b): _set("SW-157", "recomputed_this_tranche", True)(b)
    def held_next_owner_mismatch(b): _set("SW-146", "next_action_owner", "Someone else")(b)
    def held_next_owner_not_accountable(b):
        for m in b["metrics"]:
            if m["id"] == "SW-147":
                m["next_action_owner"] = "Ghost"; m["immediate_action"]["owner"] = "Ghost"
    def quarantine_use(b): _set("SW-148", "source_family_intent", "lead_source_roi")(b)
    def marker_strip(b): _set("SW-152", "missing_or_quarantine_evidence", ["something vague"])(b)
    def causal_fact(b): _set("SW-157", "business_question", "confirmed evasion present")(b)
    def held_vocab(b): _set("SW-148", "source_existence_state", "acquired_local")(b)
    def held_forbidden_key(b): _set("SW-152", "current_truth_ref", "x")(b)
    ...
    struct = [
        ...
        ("accepted_condition_drift", accepted_condition_drift), ("accepted_threshold_replace", accepted_threshold_replace),
...
def _legacy_probe_defs(legacy_now):
    ...
    return [
        ...
        ("legacy_new_error_added", base + ["ledger SW-146: source SRC-x not registered"]),
...
def main():
    ...
    receipt = {
        "artifact": "PKT-06-02_BINDING_CHECKS", "validator": VALIDATOR_REL,
