#!/usr/bin/env python3
"""PKT-05-03 J1 freeze-candidate binding validator (compact, non-recursive).

All eight targets are held source_investigation_pending. Every unproved source uses conditional
'not presently proved/found' wording (never nonexistent, never predetermined external). Known
components/corpus are supporting-only (never promoted). The CAGE aggregate is supporting-only and
quarantined (no cure; not individual/daily/causal). Current owner/status/last-edited never
substitutes for reassignment transition history (SW-197). SW-194/195/196/197/204 read no content
and require a future protected-content envelope. Capacity/handoff-friction/competence are hypotheses,
not facts.

Usage:
  python3 scripts/halo-phase1b/validate_pkt_05_03_binding.py            # writes CHECKS
  python3 scripts/halo-phase1b/validate_pkt_05_03_binding.py --no-write # validate only
"""
...
BASELINE_COMMIT = "c3f5803d2c4bf55be715f7d9432710ffa4424092"
BINDING_REL = "docs/halo/contract/phase1b/pkt-05-03-binding.json"
VALIDATOR_REL = "scripts/halo-phase1b/validate_pkt_05_03_binding.py"
CHECKS_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-05-03/PKT-05-03_BINDING_CHECKS.json"
TWO_DELTA_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-05-03/PKT-05-03_J1_TWO_DELTA.md"
ROADMAP_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-05-03/PKT-05-03_J1_internal_coverage_roadmap.md"
ALLOWLIST = [BINDING_REL, VALIDATOR_REL, CHECKS_REL, TWO_DELTA_REL, ROADMAP_REL]

IDS = ["SW-111","SW-117","SW-124","SW-194","SW-195","SW-196","SW-197","SW-204"]
ACCEPTED_IDS = []
SIP_IDS = list(IDS)
SUPPORTING_IDS = list(IDS)
CONDITIONAL_SOURCE_IDS = list(IDS)
PROTECTED_FUTURE_IDS = ["SW-194","SW-195","SW-196","SW-197","SW-204"]
MUST_NOT_SUBSTITUTE_IDS = ["SW-197"]
CAGE_ID = "SW-117"
...
JOIN_TECH_TOKENS = ("join design", "join definition", "define the join", "join implementation",
                    ...
                    "anomaly baseline implementation")
...
HELD_MARKERS = {
    "SW-111": ["presently","supporting","sales gross"],
    "SW-117": ["presently","supporting","cage"],
    "SW-124": ["presently","supporting","cohort"],
    "SW-194": ["presently","supporting","conflicting"],
    "SW-195": ["presently","supporting","introduc"],
    "SW-196": ["presently","supporting","recap"],
    "SW-197": ["presently","supporting","reassign"],
    "SW-204": ["presently","supporting","product"],
}
CAUSAL_TOKEN = {"SW-111": "capacity", "SW-117": "handoff friction", "SW-204": "competence"}
...
def run_structural(b, ctx):
    ...
    if sorted(lp.get("source_investigation_pending_ids", [])) != sorted(SIP_IDS):
        e.append("PARTITION: source_investigation_pending_ids != the eight held")
    ...
    if sorted(aa.get("ids_checked", [])) != sorted(SIP_IDS):
        e.append("AUTHORITY_ABSENCE: ids_checked != the eight held")
    ...
    for mid in IDS:
        ...
        if mid in MUST_NOT_SUBSTITUTE_IDS:
            if "substitute" not in joined or "history" not in joined:
                e.append("SUBSTITUTE %s: must state current state never substitutes for transition history" % mid)
        if mid == CAGE_ID:
            for need in ("cage", "aggregate", "quarantin"):
                if need not in joined:
                    e.append("CAGE %s: CAGE evidence must state '%s' (supporting-only, quarantined, not individual/causal)" % (mid, need))
...
def _probe_defs():
    ...
    def accepted_bucket(b): b["lifecycle_partition"]["accepted_measured_ids"] = ["SW-111"]
    def calc_bucket(b): b["lifecycle_partition"]["calculation_pending_ids"] = ["SW-111"]
    def eval_18(b): b["accepted_measured_authority"]["authoritative_evaluated_total"] = 18
    def authority_claim(b): b["authority_absence_assertion"]["in_gate5b_evaluated"] = ["SW-111"]
    def slices_break(b):
        for k in b["source_family_slices"]:
            if b["source_family_slices"][k]:
                b["source_family_slices"][k] = b["source_family_slices"][k][:-1]; break
    def held_promote(b): _set("SW-111", "disposition", "measured_validated")(b)
    def erase_supporting(b): _set("SW-124", "missing_or_quarantine_evidence",
        ["the rule is undefined; presently unproved; read-only vinsolutions check; external if needed; no proxy"])(b)
    def falsely_promote_supporting(b): _set("SW-117", "source_existence_state", "acquired_local")(b)
    def absolute_source_absence(b): _set("SW-111", "missing_or_quarantine_evidence",
        ["no trend export exists for this condition; an external source is required"])(b)
    def technical_owner_duane(b):
        for m in b["metrics"]:
            if m["id"] == "SW-124":
                m["immediate_action"] = {"owner": "Duane (x)", "action": "define the join and prove the stable-key cardinality"}
    def protected_without_authority(b): _set("SW-194", "missing_or_quarantine_evidence",
        ["the corpus is known and held as supporting context; presently unproved; read-only vinsolutions check; external if needed; no proxy"])(b)
    def proxy_join(b): _set("SW-124", "no_proxy_or_inference", False)(b)
    def cage_quarantine_cure(b): _set("SW-117", "source_family_intent", "cage_kpi")(b)
    def cage_aggregate_individual_claim(b): _set("SW-117", "missing_or_quarantine_evidence",
        ["presently unproved; read-only vinsolutions check; external if needed; no proxy; supporting context known"])(b)
    def current_state_substitution(b): _set("SW-197", "missing_or_quarantine_evidence",
        ["presently unproved; read-only vinsolutions check; external if needed; supporting context known; no proxy; content not read protected-content envelope stable identities minimization"])(b)
    def held_value(b): _set("SW-111", "value", 0)(b)
    def held_formula(b): _set("SW-124", "formula", "x/y")(b)
    def held_threshold(b): _set("SW-124", "threshold", 20)(b)
    def held_grade(b): _set("SW-111", "grade_value_or_range", "healthy")(b)
    def held_detection(b): _set("SW-204", "detection_rule", "fire")(b)
    def held_display(b): _set("SW-194", "future_display_eligibility", True)(b)
    def held_acquire(b): _set("SW-111", "direct_source_fields", ["x"])(b)
    def held_missing_zero(b): _set("SW-124", "missing_is_not_zero", False)(b)
    def held_authoritative(b): _set("SW-196", "authoritative", True)(b)
    def held_report_state(b): _set("SW-124", "report_acceptance_state", "accepted")(b)
    def held_recomputed(b): _set("SW-204", "recomputed_this_tranche", True)(b)
    def held_next_owner_mismatch(b): _set("SW-111", "next_action_owner", "Someone else")(b)
    def held_next_owner_not_accountable(b):
        for m in b["metrics"]:
            if m["id"] == "SW-117":
                m["next_action_owner"] = "Ghost"; m["immediate_action"]["owner"] = "Ghost"
    def quarantine_use(b): _set("SW-124", "source_family_intent", "lead_source_roi")(b)
    def marker_strip(b): _set("SW-124", "missing_or_quarantine_evidence", ["something vague"])(b)
    def causal_fact(b): _set("SW-204", "business_question", "confirmed competence gap present")(b)
    def held_vocab(b): _set("SW-111", "source_existence_state", "acquired_local")(b)
    def held_forbidden_key(b): _set("SW-124", "peer_rank", {"rank": 1})(b)
    ...
    struct = [
        ...
        ("proxy_join", proxy_join), ("cage_quarantine_cure", cage_quarantine_cure),
        ("cage_aggregate_individual_claim", cage_aggregate_individual_claim),
        ("current_state_substitution", current_state_substitution),
...
def _legacy_probe_defs(legacy_now):
    ...
    return [
        ...
        ("legacy_new_error_added", base + ["ledger SW-111: source SRC-x not registered"]),
...
def main():
    ...
    receipt = {
        "artifact": "PKT-05-03_BINDING_CHECKS", "validator": VALIDATOR_REL,
        ...
        "checks": {
            ...
            "protected_future_ids": PROTECTED_FUTURE_IDS, "must_not_substitute_ids": MUST_NOT_SUBSTITUTE_IDS,
            "cage_id": CAGE_ID, "slices": EXPECTED_SLICES,
