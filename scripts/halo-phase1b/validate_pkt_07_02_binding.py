#!/usr/bin/env python3
"""PKT-07-02 J1 freeze-candidate binding validator (compact, non-recursive).
...
IDs/profanity-slur text). Sentiment/NLP interpretations are provisional models for management HUMAN
REVIEW only — never automatic discipline, adverse employment, or factual claims. SW-178: an apology
count is not proof of failure.

Usage:
  python3 scripts/halo-phase1b/validate_pkt_07_02_binding.py            # writes CHECKS
  python3 scripts/halo-phase1b/validate_pkt_07_02_binding.py --no-write # validate only
"""
...
BASELINE_COMMIT = "f4016a838193369e91a144f6da7a0f586a7ed544"
BINDING_REL = "docs/halo/contract/phase1b/pkt-07-02-binding.json"
VALIDATOR_REL = "scripts/halo-phase1b/validate_pkt_07_02_binding.py"
CHECKS_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-07-02/PKT-07-02_BINDING_CHECKS.json"
TWO_DELTA_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-07-02/PKT-07-02_J1_TWO_DELTA.md"
ROADMAP_REL = "docs/halo/evidence/honda-watchdog/phase1b/pkt-07-02/PKT-07-02_J1_internal_coverage_roadmap.md"
ALLOWLIST = [BINDING_REL, VALIDATOR_REL, CHECKS_REL, TWO_DELTA_REL, ROADMAP_REL]

IDS = ["SW-170","SW-171","SW-172","SW-173","SW-174","SW-175","SW-176","SW-177","SW-178"]
ACCEPTED_IDS = []
SIP_IDS = list(IDS)
...
HELD_MARKERS = {
    "SW-170": ["presently","supporting","sentiment"], "SW-171": ["presently","supporting","punctuation"],
    "SW-172": ["presently","supporting","profanity"], "SW-173": ["presently","supporting","pushy"],
    "SW-174": ["presently","supporting","frustrat"], "SW-175": ["presently","supporting","sarcasm"],
    "SW-176": ["presently","supporting","mood"], "SW-177": ["presently","supporting","manager"],
    "SW-178": ["presently","supporting","apology"],
}
CAUSAL_TOKEN = {"SW-173": "pushy", "SW-175": "sarcasm", "SW-176": "mood-driven"}
...
def run_structural(b, ctx):
    ...
    for mid in IDS:
        ...
        if mid in SUPPORTING_IDS:
            ...
            # provisional-model / human-review / no-discipline discipline (all held)
        ...
        if mid == APOLOGY_ID:
            if "apology" not in joined or "not proof" not in joined:
                e.append("APOLOGY %s: an apology count must be stated as NOT proof of failure/wrongdoing" % mid)
...
def _probe_defs():
    ...
    def accepted_bucket(b): b["lifecycle_partition"]["accepted_measured_ids"] = ["SW-170"]
    def calc_bucket(b): b["lifecycle_partition"]["calculation_pending_ids"] = ["SW-170"]
    def eval_18(b): b["accepted_measured_authority"]["authoritative_evaluated_total"] = 18
    def authority_claim(b): b["authority_absence_assertion"]["in_gate5b_evaluated"] = ["SW-170"]
    def slices_break(b):
    ...
    _REV = "human review provisional discipline"
    def held_promote(b): _set("SW-170", "disposition", "measured_validated")(b)
    def erase_supporting(b): _set("SW-171", "missing_or_quarantine_evidence",
        ["the rule is undefined; presently unproved; read-only vinsolutions check; external if needed; no proxy; " + _PROT + "; " + _REV])(b)
    def falsely_promote_supporting(b): _set("SW-172", "source_existence_state", "acquired_local")(b)
    def absolute_source_absence(b): _set("SW-174", "missing_or_quarantine_evidence",
    ...
    def protected_without_authority(b): _set("SW-170", "missing_or_quarantine_evidence",
        ["the corpus is known and held as supporting context; labels are not stable linkage; presently unproved; read-only vinsolutions check; external if needed; no proxy; sentiment; " + _REV])(b)
    def proxy_join(b): _set("SW-171", "no_proxy_or_inference", False)(b)
    def customer_label_as_identity(b): _set("SW-177", "missing_or_quarantine_evidence",
        ["labels are stable identity; read-only vinsolutions check; external if needed; no proxy"])(b)
    def model_as_fact_discipline(b): _set("SW-173", "missing_or_quarantine_evidence",
        ["the corpus is known and held as supporting context; labels are not stable linkage; presently unproved; read-only vinsolutions check; external if needed; no proxy; " + _PROT + "; the rep is pushy and should be disciplined"])(b)
    def apology_as_proof(b): _set("SW-178", "missing_or_quarantine_evidence",
        ["the corpus is known and held as supporting context; labels are not stable linkage; presently unproved; read-only vinsolutions check; external if needed; no proxy; " + _PROT + "; " + _REV + "; the apology count proves failure"])(b)
    def held_value(b): _set("SW-170", "value", 0)(b)
    def held_formula(b): _set("SW-171", "formula", "x/y")(b)
    def held_threshold(b): _set("SW-178", "threshold", 2)(b)
    def held_grade(b): _set("SW-172", "grade_value_or_range", "healthy")(b)
    def held_detection(b): _set("SW-175", "detection_rule", "fire")(b)
    def held_display(b): _set("SW-170", "future_display_eligibility", True)(b)
    def held_acquire(b): _set("SW-170", "direct_source_fields", ["x"])(b)
    def held_missing_zero(b): _set("SW-171", "missing_is_not_zero", False)(b)
    def held_authoritative(b): _set("SW-176", "authoritative", True)(b)
    def held_report_state(b): _set("SW-177", "report_acceptance_state", "accepted")(b)
    def held_recomputed(b): _set("SW-178", "recomputed_this_tranche", True)(b)
    def held_next_owner_mismatch(b): _set("SW-170", "next_action_owner", "Someone else")(b)
    def held_next_owner_not_accountable(b):
        for m in b["metrics"]:
            if m["id"] == "SW-171":
                m["next_action_owner"] = "Ghost"; m["immediate_action"]["owner"] = "Ghost"
    def quarantine_use(b): _set("SW-173", "source_family_intent", "lead_source_roi")(b)
    def marker_strip(b): _set("SW-172", "missing_or_quarantine_evidence", ["something vague"])(b)
    def causal_fact(b): _set("SW-173", "business_question", "confirmed pushy present")(b)
    def held_vocab(b): _set("SW-171", "source_existence_state", "acquired_local")(b)
    def held_forbidden_key(b): _set("SW-172", "current_truth_ref", "x")(b)
    ...
    struct = [
        ...
        ("model_as_fact_discipline", model_as_fact_discipline), ("apology_as_proof", apology_as_proof),
...
def _legacy_probe_defs(legacy_now):
    ...
    return [
        ...
        ("legacy_new_error_added", base + ["ledger SW-170: source SRC-x not registered"]),
...
def main():
    ...
    receipt = {
        "artifact": "PKT-07-02_BINDING_CHECKS", "validator": VALIDATOR_REL,
        ...
        "checks": {
            ...
            "protected_future_ids": PROTECTED_FUTURE_IDS, "apology_id": APOLOGY_ID,
